...
/**
  ...
  * 参    数：无
  * 返 回 值：无
  * 说    明：PA0(SDA)和PA1(SCL)已由SysConfig配置为硬件I2C1外设功能，
  *           此处仅做上电延时确保OLED供电稳定
  */
...
/**
  ...
  * 参    数：dat  要写入OLED的数据
  * 参    数：mode 写入模式：0=命令(发0x00前缀)，1=数据(发0x40前缀)
  * 返 回 值：无
  * 说    明：通过硬件I2C1发送 [控制字节, 数据字节]，共2字节。
  *           参考oled.c已验证的等待序列：
  *           IDLE → 填FIFO → 启动传输 → BUSY_BUS确认启动 → IDLE等待完成
  */
...
{
	OLED_WR_Byte(Command, 0);	/* mode=0 → 控制字节0x00（命令模式） */
}
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
	...
	for (i = 0; i < Count; i ++)
	{
		OLED_WR_Byte(Data[i], 1);	/* mode=1 → 控制字节0x40（数据模式） */
	}
}
