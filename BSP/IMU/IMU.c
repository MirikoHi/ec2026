...
// 方差递推公式 S^2 = (X1^2 + X2^2 + X3^2 + ... +Xn^2)/n - X平均^2
// 函数名称: calVariance
// 功能描述: 计算方差
// 输入参数: data[] --- 进行方差计算的数据缓冲区
//           length --- 数据长度
// 输出参数:                                                                                      */
//           sqrResult[] --- 方差结果
//           avgResult[] --- 平均值
...
//  bz = hz;
  ...
  /* 加速度：由北向南的加速度在加速度计X轴投影 */
	north.x = 1 - 2*(q3*q3 + q2*q2);
	/* 加速度：由北向南的加速度在加速度计Y轴投影 */
	north.y = 2* (-q0*q3 + q1*q2);
	/* 加速度：由北向南的加速度在加速度计Z轴投影 */
	north.z = 2* (+q0*q2  - q1*q3);
	/* 加速度：由东向西的加速度在加速度计X轴投影 */
	west.x = 2* (+q0*q3 + q1*q2);
	/* 加速度：由东向西的加速度在加速度计Y轴投影 */
	west.y = 1 - 2*(q3*q3 + q1*q1);
	/* 加速度：由东向西的加速度在加速度计Z轴投影 */
...
*功能说明:	 更新四元数 返回当前四元数的值
*输入参数： 需要放置四元数的数组首地址
*输出参数：无
*******************************************************************************/
float mygetqval[9];	//用于存放从传感器读取的数据
...
void IMU_getYawPitchRoll(float * angles) {
  float q[4]; //四元数
  volatile float gx=0.0, gy=0.0, gz=0.0; //姿态角变量
  IMU_getQ(q); //更新全局四元数
