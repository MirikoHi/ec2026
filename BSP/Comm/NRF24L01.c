...
//
// __IO FIFO_t NRF24L01_rxFIFO={0};
// __IO FIFO_t NRF24L01_txFIFO={0};
//
// /**
//   * 提示：设备A和设备B进行通信
...
//
//
// volatile uint8_t NRF24L01_Receive_Success = 0;
...
// //        case NRF24_IRQ_IIDX:
// //
// //            if( !DL_GPIO_readPins(NRF24_IRQ_PORT, NRF24_IRQ_PIN) )
// //            {
// //								status=NRF24L01_ReadReg(NRF24L01_STATUS);
// //                if(NRF24L01_Receive()==1)
// //								{
// //									for(uint8_t i =1;i<NRF24L01_RX_PACKET_WIDTH&&i<=NRF24L01_RxPacket[0];i++)
// //									{
// //										fifo_enQueue(&NRF24L01_rxFIFO,NRF24L01_RxPacket[i]);
// //										fifo_enQueue(&NRF24L01_txFIFO,NRF24L01_RxPacket[i]);
// //									}
// //								}
// //								NRF24L01_WriteReg(NRF24L01_STATUS, status);	     //清除07寄存器标志
// //            }
// //
// //        break;
// //
...
// {
// 	//发送
// 	if((!fifo_isEmpty(&NRF24L01_txFIFO)))
// 	{
//
// 		uint8_t queueLength = fifo_queueLength(&NRF24L01_txFIFO);
//
// 		while(queueLength!=0)
// 		{
// 			uint8_t this_send_queuelength = queueLength>(NRF24L01_TX_PACKET_WIDTH-1)?(NRF24L01_TX_PACKET_WIDTH-1):queueLength;
// 			for(uint8_t i=0;i<NRF24L01_TX_PACKET_WIDTH;i++)
// 			{
// 				NRF24L01_TxPacket[i] = 0;
// 			}
// 			NRF24L01_TxPacket[0] = queueLength;
// 			for(uint8_t i=1;i<NRF24L01_TX_PACKET_WIDTH&&i<=this_send_queuelength;i++)
// 			{
// 				NRF24L01_TxPacket[i] = fifo_deQueue(&NRF24L01_txFIFO);
// 				queueLength--;
// 			}
// 			while(NRF24L01_Send()!=1);
// 		}
// 	}
// 	else
// 	{
// 			for(uint8_t i=0;i<NRF24L01_TX_PACKET_WIDTH;i++)
// 			{
// 				NRF24L01_TxPacket[i] = 0xff;
// 			}
// 			NRF24L01_Send();
// 	}
// //	DWT_Delay(0.0000002);
// 	//接收
// 	NRF24L01_Receive_Success = NRF24L01_Receive();
// 	float start_time = DWT_GetTimeline_ms();
// 		while(NRF24L01_Receive_Success!=1)
// 		{
// 			NRF24L01_Receive_Success = NRF24L01_Receive();
// 			if((DWT_GetTimeline_ms()-start_time)>0.5)
// 			{
// 				return;
// 			}
// 		}
...
// }
//
// void NRF24L01_FIFO_Init(void)
// {
// 	fifo_initQueue(&NRF24L01_rxFIFO);
// 	fifo_initQueue(&NRF24L01_txFIFO);
...
// 	if(BitValue) {
//         DL_GPIO_setPins(NRF24_CE_PORT, NRF24_CE_PIN);   // 设置高电平
//     } else {
//         DL_GPIO_clearPins(NRF24_CE_PORT, NRF24_CE_PIN); // 设置低电平
//     }
// }
//
// /**
...
// 	if(BitValue) {
//         DL_GPIO_setPins(NRF24_CSN_PORT, NRF24_CSN_PIN);   // 设置高电平
//     } else {
//         DL_GPIO_clearPins(NRF24_CSN_PORT, NRF24_CSN_PIN); // 设置低电平
//     }
// }
//
// /**
...
// 	if(BitValue) {
//         DL_GPIO_setPins(NRF24_SCK_PORT, NRF24_SCK_PIN);   // 设置高电平
//     } else {
//         DL_GPIO_clearPins(NRF24_SCK_PORT, NRF24_SCK_PIN); // 设置低电平
//     }
// }
//
// /**
...
// 	if(BitValue) {
//         DL_GPIO_setPins(NRF24_MOSI_PORT, NRF24_MOSI_PIN);   // 设置高电平
//     } else{
//         DL_GPIO_clearPins(NRF24_MOSI_PORT, NRF24_MOSI_PIN); // 设置低电平
//     }
// }
//
// /**
...
// 	bool i = 0;
// 	if(DL_GPIO_readPins(NRF24_MISO_PORT,NRF24_MISO_PIN) > 0)i=1;
// 	else if(DL_GPIO_readPins(NRF24_MISO_PORT,NRF24_MISO_PIN) < 0)i=0;
// 	return i;
// }
//
// /**
...
// /**
//   * 函    数：发送指定的数据包并更新计数器
//   * 参    数：pData - 指向要发送的数据数组的指针
//   *           pSuccessCount - 指向发送成功计数器的指针
//   *           pFailedCount  - 指向发送失败计数器的指针
//   * 返 回 值：发送状态标志 (1:成功, 2:最大重发, 3:状态错误, 4:超时)
//   */
// uint8_t NRF24L01_SendPacketWithCount(uint8_t *pData, uint32_t* pSuccessCount, uint32_t* pFailedCount)
// {
//     uint8_t SendFlag;
//
//     /* 将输入数据复制到发送缓冲区 */
//     memcpy(NRF24L01_TxPacket, pData, NRF24L01_TX_PACKET_WIDTH);
//
//     /* 发送数据 */
